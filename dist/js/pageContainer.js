...
(function () {
    ...
    var view = {
        ...
        find: function find(selectorOrDom) {
            return view.el.find(selectorOrDom);
        },
        show: function show($dom) {
            this.find('section').map(function (index, item) {
                $(item).addClass('hide');
            });
        ...
        jumpToBottom: function jumpToBottom() {
            ...
            $("html, body").animate({ scrollTop: this.el[0].offsetHeight }, 500, function () {
                var selfInfo = _this.find('.self-info');
                selfInfo.addClass('jump');
                setTimeout(function () {
                    selfInfo.removeClass('jump');
                }, 1000);
            });
        }
    };
    ...
    var controller = {
        ...
        bindEvents: function bindEvents() {
            this.playMusic();
            this.initSwiper();
            EventsHub.subscribe('switchTab', function (data) {
        ...
        playMusic: function playMusic() {
            view.el.on('click', '.item-common', function (event) {
                var $clickedli = view.find(event.currentTarget);
                model.refreshData({
                    id: $clickedli.attr('id').trim(),
                    song: $clickedli.find('.song').text().trim(),
                    singer: $clickedli.find('.singer').text().trim(),
                    lyric: $clickedli.find('.lyric').text().trim(),
                    url: $clickedli.find('.url').text().trim(),
                    cover: $clickedli.find('.cover').attr('src').trim()
                });

...
(function () {
    ...
    var view = {
        el: $('#page-play'),
        render: function render($el, data, empty) {
            if (empty) {
    ...
    var model = {
        ...
        refreshAudio: function refreshAudio(audio, data) {
            var _splitLyricAndTime = this.splitLyricAndTime(data),
                lyric = _splitLyricAndTime.lyric,
                time = _splitLyricAndTime.time,
                timeline = _splitLyricAndTime.timeline;
        ...
        splitLyricAndTime: function splitLyricAndTime(lyric) {
            var audio = {
                lyric: [],
                time: [],
                timeline: []
            ...
            audio.time.map(function (time, index, arrayTime) {
        ...
        creatLyricLine: function creatLyricLine(audio) {
            this.templateLyrics = audio.lyric.map(function (eachLine, index) {
                var line = $('<p></p>');
    ...
    var controller = {
        ...
        bindAudioEvent: function bindAudioEvent() {
            ...
            view.find('.audio').on('canplay', function () {
                ...
                _this2.previousMusic();
                _this2.nextMusic();
                view.changePlayPageBackground(model.data);
                _this2.showPlayPage();
            });
            view.find('.audio').on('timeupdate', function () {
                var time = view.find('.audio')[0].currentTime;
                var lyrics = view.find('.lyric p');
                lyrics.map(function (index, eachLine) {
                    if (index !== lyrics.length - 1) {
                        var _currentTime = lyrics.eq(index).attr('data-time');
                        var nextTime = lyrics.eq(index + 1).attr('data-time');
                        if (_currentTime <= time && time < nextTime) {
                            if (lyrics.eq(index - 1)) {
                                lyrics.eq(index - 1).removeClass('active');
                            }
                            var pHeight = eachLine.getBoundingClientRect().top;
                            var lyricHeight = view.find('.lyric')[0].getBoundingClientRect().top;
                            var height = pHeight - lyricHeight;
        ...
        previousMusic: function previousMusic() {
            view.find('.prev-btn').on('click', function () {
                EventsHub.publish('previousMusic', model.data.id);
            });
        },
        nextMusic: function nextMusic() {
            view.find('.next-btn').on('click', function () {

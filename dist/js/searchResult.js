...
(function () {
    ...
    var model = {
        ...
        notFoundTemplate: '\n            <li class="not-found">\u563F\u563F\uFF0C\u6CA1\u6709\u4F60\u8981\u542C\u7684\u6B4C~ \uD83E\uDD23</li>\n        ',
        temporaryTemplate: [],
        ...
        fetchLeanCloudData: function fetchLeanCloudData() {
            ...
            query.find().then(function (songs) {
                _this.leanCloud = songs.map(function (song) {
                    return {
                        id: song.id,
        ...
        queryLeanCloudData: function queryLeanCloudData(songName) {
            var _this2 = this;

            var song = new AV.Query('SongList');
            return song.contains('song', songName).find().then(function (songs) {
                if (songs.length !== _this2.leanCloud.length && songs.length !== 0) {
                    _this2.tempData = songs.map(function (song) {
                        return {
                            id: song.id,
                            song: song.attributes.song,
                            singer: song.attributes.singer,
                    ...
                    return [_this2.notFoundTemplate];
                } else {
                    return undefined;
                }
            });
        ...
        showLeanCloudData: function showLeanCloudData() {
            ...
            this.tempData.map(function (song) {
    ...
    var controller = {
        init: function init() {
            model.fetchLeanCloudData();
            this.bindEvents();
        },
        bindEvents: function bindEvents() {
            EventsHub.subscribe('input', function (data) {
                model.queryLeanCloudData(data).then(function (data) {
                    if (data) {
                        view.render(view.find('.song-list'), data);

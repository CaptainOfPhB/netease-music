...
(function () {
    ...
    let view = {
        ...
        show($dom) {
            this.find('section').map((index, item) => {
                $(item).addClass('hide');
            });
        ...
        jumpToBottom() {
            $("html, body").animate({scrollTop: this.el[0].offsetHeight}, 500, () => {
                let selfInfo = this.find('.self-info');
    ...
    let controller = {
        ...
        bindEvents() {
            this.playMusic();
            this.initSwiper();
            EventsHub.subscribe('switchTab', (data) => {
        ...
        playMusic() {
            view.el.on('click', '.item-common', (event) => {
                let $clickedli = view.find(event.currentTarget);
                model.refreshData({

...
(function () {
    ...
    let model = {
        ...
        queryLeanCloudData(songName) {
            let song = new AV.Query('SongList');
            return song.contains('song', songName).find().then(
                (songs) => {
                    if (songs.length !== this.leanCloud.length && songs.length !== 0) {
                        this.tempData = songs.map((song) => {
                            return {
                                id: song.id,
                                song: song.attributes.song,
                                singer: song.attributes.singer,
    ...
    let controller = {
        ...
        bindEvents() {
            EventsHub.subscribe('input', (data) => {
                model.queryLeanCloudData(data).then(
                    (data) => {
                        if (data) {
